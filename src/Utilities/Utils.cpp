	...
	const std::string& target)
{
  ...
  for( char c : filename )
    ws += c;
	hr = D3DCompileFromFile(ws.c_str(), defines, D3D_COMPILE_STANDARD_FILE_INCLUDE,
...
D3D12_SHADER_BYTECODE d3dUtil::getShaderBytecode(ID3DBlob* blob) {
    D3D12_SHADER_BYTECODE byteCode = { reinterpret_cast<BYTE*>(blob->GetBufferPointer()), blob->GetBufferSize() };
    return byteCode;
}
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource>& uploadBuffer)
{
    ...
    // Create the actual default buffer resource.
        CD3DX12_HEAP_PROPERTIES defaultHP(D3D12_HEAP_TYPE_DEFAULT);
        auto resDesc = CD3DX12_RESOURCE_DESC::Buffer(byteSize);
    ...
    auto transition1 = CD3DX12_RESOURCE_BARRIER::Transition(defaultBuffer.Get(),
...
ComPtr<ID3DBlob> d3dUtil::LoadBinary(const std::string& filename)
{
