...
//
//
Texture::Texture(const std::string& filename, UINT64 _textureID) : Filename(filename), textureID(_textureID){

} 

bool Texture::createTextureBuffer(ID3D12Device* device) {
	std::wstring w = ToWString( Filename );
	std::wstring w2;
	std::wstring w3;

	// hack time
	if( w.ends_with( L"tga" ) || w.ends_with( L"TGA" ) )
	{
		{
			// pop the ".tga"
			w.pop_back();
		...
		w3 = w;
		w3 += L".png";
		std::wstring f = w.substr( w.find_last_of( L"/\\" ) + 1 );
	...
	int imageSize = 0;
	const std::wstring ws[] = { w, w2, w3 };
	for( const std::wstring& currW : ws )
	{
    imageSize = LoadImageDataFromFile(&imageData, textureDESC, currW.c_str(), imageBytesPerRow);
    if( imageSize > 0 )
    {
      break;
    }
  ...
  if( imageSize <= 0 )
  {
	...
	for( char c : Filename )
		wname += ( wchar_t )c;
	textureBuffer->SetName(wname.c_str());
	return true;

}

bool Texture::uploadTexture(ID3D12Device* device, CommandObject* cmdObj) {
	...
	// upload buffer
		...
		auto rscDsc = CD3DX12_RESOURCE_DESC::Buffer(textureUploadBufferSize);
	ThrowIfFailed(device->CreateCommittedResource(
		&uploadHP,
		D3D12_HEAP_FLAG_NONE,
		&rscDsc,
	...
	std::wstring w = ToWString( "texture upload buffer" );
	UploadHeap->SetName(w.c_str());
	...
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition( textureBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
		cmdObj->getCommandListPtr()->ResourceBarrier( 1, &barrier );
	}
...
RenderTexture::RenderTexture(UINT _width, UINT _height, DXGI_FORMAT _format) : width(_width), height(_height){
	...
	textureDESC.Format = _format; // This is the dxgi format of the image (format of the pixels)
	...
	depthStencilDESC.Alignment = 0; // may be 0, 4KB, 64KB, or 4MB. 0 will let runtime decide between 64KB and 4MB (4MB for multi-sampled textures)
	depthStencilDESC.Width = width; // width of the texture
	depthStencilDESC.Height = height; // height of the texture
	depthStencilDESC.DepthOrArraySize = 1; // if 3d image, depth of 3d image. Otherwise an array of 1D or 2D textures (we only have one image, so we set 1)
	depthStencilDESC.MipLevels = 0; // Number of mipmaps. We are not generating mipmaps for this texture, so we have only one level
	depthStencilDESC.Format = DXGI_FORMAT_R24G8_TYPELESS; // This is the dxgi format of the image (format of the pixels)
	depthStencilDESC.SampleDesc.Count = 1; // This is the number of samples per pixel, we just want 1 sample
	depthStencilDESC.SampleDesc.Quality = 0; // The quality level of the samples. Higher is better quality, but worse performance
	depthStencilDESC.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN; // The arrangement of the pixels. Setting to unknown lets the driver choose the most efficient one
...
bool RenderTexture::uploadTexture(ID3D12Device* device, CommandObject* cmdObj) {
	return true;
}

bool RenderTexture::initializeTextureBuffer(ID3D12Device* device, CommandObject* cmdObj) {
	if (!(createTextureBuffer(device))) {
