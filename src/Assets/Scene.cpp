...
void Scene::loadTextures() {
    ...
    auto mytex1 = std::make_unique<Texture>("Resources/Textures/r1.png", globalTextureSRVDescriptorHeapIndex++);
    mytex1->Name = "tex1";
    mytex1->initializeTextureBuffer(md3dDevice, cpyCommandObject.get());

    auto mytex2 = std::make_unique <Texture>("Resources/Textures/r2.png", globalTextureSRVDescriptorHeapIndex++);
    mytex2->Name = "tex2";
    mytex2->initializeTextureBuffer(md3dDevice, cpyCommandObject.get());

    auto mytex3 = std::make_unique <Texture>("Resources/Textures/r3.png", globalTextureSRVDescriptorHeapIndex++);
    mytex3->Name = "tex3";
   ...
   // importer.SetPropertyInteger(AI_CONFIG_PP_SBP_REMOVE, aiPrimitiveType_LINE | aiPrimitiveType_POINT);
    ...
    for (unsigned int i = 0; i < aiscene->mNumMaterials; ++i) {
        ...
        for (unsigned int j = 0; j < curMaterial->GetTextureCount(aiTextureType_DIFFUSE); ++j) {
            ...
            auto curtex = std::make_unique<Texture>(completeTexturePath, globalTextureSRVDescriptorHeapIndex);
            curtex->Name = completeTexturePath;
            curtex->Name = curtex->Name.substr(curtex->Name.find_last_of('\\') + 1);
            curtex->Name = curtex->Name.substr(curtex->Name.find_last_of('/') + 1);
