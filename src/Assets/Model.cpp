...
void Model::createBuffers(ID3D12Device* device) {
  ...
  CD3DX12_HEAP_PROPERTIES hpDefault( D3D12_HEAP_TYPE_DEFAULT );
        auto vbrscDsc = CD3DX12_RESOURCE_DESC::Buffer(VertexBufferByteSize);
        auto ibrscDsc = CD3DX12_RESOURCE_DESC::Buffer(IndexBufferByteSize);
    ThrowIfFailed(device->CreateCommittedResource(
        &hpDefault,// this is a default heap
...
void Model::uploadBuffers(ID3D12Device* device, CommandObject* cmdObj) {
        ...
        const CD3DX12_HEAP_PROPERTIES hpUpload(D3D12_HEAP_TYPE_UPLOAD);
        auto vbDesc = CD3DX12_RESOURCE_DESC::Buffer(VertexBufferByteSize);
    ThrowIfFailed(device->CreateCommittedResource(
        &hpUpload,
        D3D12_HEAP_FLAG_NONE,
        &vbDesc,
        ...
        auto ibDesc = CD3DX12_RESOURCE_DESC::Buffer(IndexBufferByteSize);
    ThrowIfFailed(device->CreateCommittedResource(
        &hpUpload,
        D3D12_HEAP_FLAG_NONE,
        &ibDesc,
    ...
    {
      auto transition = CD3DX12_RESOURCE_BARRIER::Transition( VertexBufferGPU.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_GENERIC_READ );
      cmdObj->getCommandListPtr()->ResourceBarrier( 1, &transition );
    }

    UpdateSubresources<1>(cmdObj->getCommandListPtr(), IndexBufferGPU.Get(), IndexBufferUploader.Get(), 0, 0, 1, &subResourceDataIndex);
    {
      auto transition = CD3DX12_RESOURCE_BARRIER::Transition( IndexBufferGPU.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_GENERIC_READ );
      cmdObj->getCommandListPtr()->ResourceBarrier( 1, &transition );
    }
