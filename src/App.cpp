...
bool App::InitImgui() {
    ...
    g_pd3dSrvDescHeapAlloc.Create( md3dDevice.Get(), m_cbvSrvHeap4Imgui.Get() );
    ...
    imguiInitInfo.Device = md3dDevice.Get();
    imguiInitInfo.CommandQueue = mCommandQueue.Get();
    imguiInitInfo.NumFramesInFlight = d3dUtil::gNumFrameResources;
    imguiInitInfo.RTVFormat = mBackBufferFormat;
    imguiInitInfo.DSVFormat = mDepthStencilFormat;
    imguiInitInfo.SrvDescriptorHeap = m_cbvSrvHeap4Imgui.Get();
...
void App::UpdateGui(const Timer& gt) {
    ...
    // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
    {
        ...
        ImGui::Text( "Tab - Toggle camera lock (current status: %s)", isFreeCamEnabled ? "unlocked" : "locked" );
...
const int kDirBufSize = 1024;
static char sOldDir[ kDirBufSize ];
...
bool App::Initialize() {
    ...
    if( mScene->name == "sponza.obj")
    {
      DirectX::XMFLOAT4X4* World = &mScene->getObjectInfos()[ "sponza.obj" ]->World;
      DirectX::XMMATRIX scaling = DirectX::XMMatrixScaling( modelScale, modelScale, modelScale );
      DirectX::XMStoreFloat4x4(World, scaling);
      if( Camera* mainCam = mScene->getCamerasMap()[ "MainCam" ].get() )
      {
        mainCam->SetPosition( 101.366760f, 66.5440979f, -14.3385210f );
        mainCam->SetRight( 0.0156098418f, 8.33533704e-08f, 0.999878168f );
        mainCam->SetUp( -0.407187939f, 0.913322270f, 0.00635683769f );
        mainCam->SetLook( -0.913210988f, -0.407237560f, 0.0142568499f );
      }
    }
...
void App::Update(const Timer& gt) {
    ...
    {
        HANDLE eventHandle = CreateEventEx(nullptr, "d", false, EVENT_ALL_ACCESS);
...
void App::UpdateMainPassCB(const Timer& gt)
{
    ...
    auto projDet = DirectX::XMMatrixDeterminant( proj );
...
{
    mShaders["standardVS"] = d3dUtil::CompileShader("Resources/Shaders/pix.hlsl", nullptr, "VS", "vs_5_1");
    mShaders["opaquePS"] = d3dUtil::CompileShader("Resources/Shaders/pix.hlsl", nullptr, "PS", "ps_5_1");
    mShaders["shadowVS"] = d3dUtil::CompileShader("Resources/Shaders/shadow.hlsl", nullptr, "VS", "vs_5_1");
    mShaders["shadowOpaquePS"] = d3dUtil::CompileShader("Resources/Shaders/shadow.hlsl", nullptr, "PS", "ps_5_1");
    mShaders["debugVS"] = d3dUtil::CompileShader("Resources/Shaders/shadowDebug.hlsl", nullptr, "VS", "vs_5_1");
    mShaders["debugPS"] = d3dUtil::CompileShader("Resources/Shaders/shadowDebug.hlsl", nullptr, "PS", "ps_5_1");
    mShaders["deferredVS"] = d3dUtil::CompileShader("Resources/Shaders/deferred.hlsl", nullptr, "VS", "vs_5_1");
    mShaders["deferredPS"] = d3dUtil::CompileShader("Resources/Shaders/deferred.hlsl", nullptr, "PS", "ps_5_1");

    mShaders["voxelizerVS"] = d3dUtil::CompileShader("Resources/Shaders/voxelizer.hlsl", nullptr, "VS", "vs_5_1");
    mShaders["voxelizerGS"] = d3dUtil::CompileShader("Resources/Shaders/voxelizer.hlsl", nullptr, "GS", "gs_5_1");
    mShaders["voxelizerPS"] = d3dUtil::CompileShader("Resources/Shaders/voxelizer.hlsl", nullptr, "PS", "ps_5_1");
    mShaders["voxelizerCompReset"] = d3dUtil::CompileShader("Resources/Shaders/voxelizer.hlsl", nullptr, "CompReset", "cs_5_1");
    mShaders["radianceCS"] = d3dUtil::CompileShader("Resources/Shaders/radiance.hlsl", nullptr, "Radiance", "cs_5_1");
    mShaders["CompFillBaseMip"] = d3dUtil::CompileShader("Resources/Shaders/baseMipRadiance.hlsl", nullptr, "BaseMipRadiance", "cs_5_1");
    mShaders["CompFillAllMip"] = d3dUtil::CompileShader("Resources/Shaders/baseMipRadiance.hlsl", nullptr, "AllMipRadiance", "cs_5_1");
    mInputLayout =
    {
...
void App::CreatePSO(
    ...
    auto hr = md3dDevice->CreateGraphicsPipelineState( &PsoDesc, IID_PPV_ARGS( pso ) );
    if( FAILED( hr ) )
    {
      __debugbreak();
    }
